...
Xmart_chats = []
en_chats = []
# AI Chat (C) 2020-2021 by @XmartPerson


@Xmart.on_message(filters.command("chatbot") & ~filters.edited & ~filters.bot)
@admins_only
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Xmart AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Xmart AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Xmart AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Xmart AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        result = result.replace("Aco", "Xmart")
        result = result.replace("<a href=\\", "<a href =")
        result = result.replace("<\/a>", "</a>")
        pro = result
        try:
            await Xmart.send_chat_action(message.chat.id, "typing")
...
@Xmart.on_message(filters.text & filters.private & ~filters.reply & ~filters.bot)
...
@Xmart.on_message(
    filters.regex("Xmart|Xmart|warlegend|hello|hi")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
